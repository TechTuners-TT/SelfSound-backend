...
MAX_VIDEO_SIZE = 250 * 1024 * 1024  # 100MB
...
async def upload_file_to_storage(file: UploadFile, file_path: str) -> str:
    ...
    try:
        logger.info(f"Starting upload: {file_path} (content-type: {file.content_type})")
        ...
        # Check Supabase file size limit (50MB for free tier)
        if file_size > 50 * 1024 * 1024:
            logger.error(f"File too large for Supabase: {file_size / (1024 * 1024):.2f}MB")
            raise HTTPException(
                status_code=413,
                detail=f"File size ({file_size / (1024 * 1024):.1f}MB) exceeds Supabase limit of 50MB"
            )

        # Upload to Supabase storage
        logger.info(f"Uploading to bucket: {STORAGE_BUCKET}")
        ...
        if hasattr(response, 'error') and response.error:
            ...
            error = response['error']
        elif isinstance(response, dict) and response.get('statusCode', 200) >= 400:
            error = response.get('message', 'Upload failed')

        if error:
            logger.error(f"Storage upload error: {error}")
            error_detail = str(error)
            if isinstance(error, dict):
                error_detail = error.get('message', str(error))
                if 'payload too large' in error_detail.lower() or 'entity too large' in error_detail.lower():
                    error_detail = "File too large for Supabase storage"
                elif 'unauthorized' in error_detail.lower():
                    error_detail = "Storage access denied - check bucket permissions"
                elif 'quota' in error_detail.lower() or 'limit' in error_detail.lower():
                    error_detail = "Storage quota exceeded"

            raise HTTPException(status_code=500, detail=f"Failed to upload file to storage: {error_detail}")
        ...
        try:
            ...
            if isinstance(public_url_response, dict):
                public_url = (public_url_response.get('publicUrl') or
                              public_url_response.get('data', {}).get('publicUrl') or
                              public_url_response.get('publicURL'))
            elif isinstance(public_url_response, str):
                public_url = public_url_response
            else:
                public_url = str(public_url_response) if public_url_response else None

            if not public_url or public_url == 'None' or public_url == '':
                logger.error(f"Failed to get public URL. Response: {public_url_response}")
        ...
        if 'timeout' in error_msg.lower():
            error_msg = "Upload timed out - file may be too large"
        elif 'connection' in error_msg.lower():
            error_msg = "Connection error - please check your internet connection"
        elif 'permission' in error_msg.lower() or 'unauthorized' in error_msg.lower():
            error_msg = "Storage access denied - check bucket permissions"
...
async def upload_audio_to_storage(file: UploadFile, file_path: str) -> str:
    ...
    try:
        logger.info(f"Starting audio upload: {file_path} (content-type: {file.content_type})")
        ...
        # Check Supabase file size limit (50MB for free tier)
        if file_size > 50 * 1024 * 1024:
            logger.error(f"Audio file too large for Supabase: {file_size / (1024 * 1024):.2f}MB")
            raise HTTPException(
                status_code=413,
                detail=f"Audio file size ({file_size / (1024 * 1024):.1f}MB) exceeds Supabase limit of 50MB"
            )

        # Upload to Supabase storage
        logger.info(f"Uploading to audio bucket: {AUDIO_STORAGE_BUCKET}")
        ...
        logger.info(f"Audio upload response: {response}")
        ...
        if error:
            logger.error(f"Audio storage upload error: {error}")
            error_detail = str(error)
            if isinstance(error, dict):
                error_detail = error.get('message', str(error))
                if 'payload too large' in error_detail.lower() or 'entity too large' in error_detail.lower():
                    error_detail = "Audio file too large for Supabase storage"
                elif 'unauthorized' in error_detail.lower():
                    error_detail = "Storage access denied - check bucket permissions"
                elif 'quota' in error_detail.lower() or 'limit' in error_detail.lower():
                    error_detail = "Storage quota exceeded"

            raise HTTPException(status_code=500, detail=f"Failed to upload audio file to storage: {error_detail}")

        logger.info("Audio upload successful, getting public URL...")

        # Get public URL
        try:
            public_url_response = supabase.storage.from_(AUDIO_STORAGE_BUCKET).get_public_url(file_path)
            ...
            if not public_url or public_url == 'None' or public_url == '':
                logger.error(f"Failed to get public URL for audio. Response: {public_url_response}")
                raise HTTPException(status_code=500, detail="Failed to get public URL for uploaded audio file")

            logger.info(f"Successfully uploaded audio: {file_path} -> {public_url}")
            return public_url

        except Exception as url_error:
            logger.error(f"Error getting audio public URL: {str(url_error)}")
            # Try to clean up the uploaded file
            try:
                supabase.storage.from_(AUDIO_STORAGE_BUCKET).remove([file_path])
                logger.info(f"Cleaned up failed audio upload: {file_path}")
            except Exception as cleanup_error:
        ...
        error_msg = str(e)
        if 'timeout' in error_msg.lower():
            error_msg = "Audio upload timed out - file may be too large"
        elif 'connection' in error_msg.lower():
            error_msg = "Connection error - please check your internet connection"
        elif 'permission' in error_msg.lower() or 'unauthorized' in error_msg.lower():
            error_msg = "Storage access denied - check bucket permissions"

        raise HTTPException(status_code=500, detail=f"Audio file upload failed: {error_msg}")


# Debug endpoint to test Supabase uploads directly
...
async def test_upload_endpoint(
    ...
    try:
        logger.info(f"DEBUG: Testing upload for {file.filename}")
        ...
        # Generate test path
        test_path = f"debug/{current_user['id']}/test_{uuid.uuid4().hex[:8]}_{file.filename}"

        logger.info(f"DEBUG: File size: {file_size / (1024 * 1024):.2f}MB")
        logger.info(f"DEBUG: Test path: {test_path}")

        # Try the upload
        try:
            public_url = await upload_file_to_storage(file, test_path)

            # Clean up test file
            try:
                supabase.storage.from_(STORAGE_BUCKET).remove([test_path])
                logger.info(f"DEBUG: Cleaned up test file")
            except:
                pass
